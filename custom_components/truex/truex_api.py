"""TrueX OPENAPI CUBE REST client using grant_type=1 with HMAC-SHA256 signing."""
...
class TrueXAPI:
    ...
    async def get_access_token(self) -> dict[str, Any]:
        ...
        _LOGGER.debug("Getting access token from %s", url)

        async with session.get(url, headers=headers) as resp:
            response = await resp.json()

        _LOGGER.debug("Token response: %s", response)
    ...
    async def refresh_access_token(self) -> dict[str, Any]:
        ...
        _LOGGER.warning("Token refresh failed, getting new token: %s", response)
        return await self.get_access_token()
    ...
    async def request(
        ...
        if not response.get("success"):
            ...
            if code in (1010, "1010"):
                _LOGGER.debug("Token invalid, refreshing and retrying...")
                await self.get_access_token()
                headers = self._build_headers(method, path, query_params, body_str)
                ...
                _LOGGER.warning("API error: %s (code: %s)", msg, code)
